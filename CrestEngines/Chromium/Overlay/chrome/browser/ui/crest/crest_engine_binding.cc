...
void CREST_CALL EngineBinding::Run(void* context, const uint8_t* command, size_t length) {
  ...
  if (!content::BrowserThread::CurrentlyOn(content::BrowserThread::UI)) {
    content::GetUIThreadTaskRunner({})->PostTask(
        FROM_HERE, base::BindOnce(&EngineBinding::Handle, binding->weak_factory_.GetWeakPtr(), std::move(*decoded)));
    return;
  }
  binding->Handle(std::move(*decoded));
}

void EngineBinding::Handle(engine::EngineCommand command) {
  if (disposing_) {
    return;
  }
...
void EngineBinding::Create(const engine::CreatePage& creation) {
  ...
  pages_.emplace(key, std::make_unique<EnginePage>(*this, creation));
...
bool EngineBinding::Restore(const std::string& key, std::vector<uint8_t> state, const std::string& expected_url) {
  EnginePage* page = Find(key);
  return page && page->Restore(std::move(state), expected_url);
}

std::optional<std::vector<uint8_t>> EngineBinding::SaveInteractionState(const std::string& key) {
  EnginePage* page = Find(key);
  return page ? page->SaveInteractionState() : std::nullopt;
}

std::optional<std::vector<uint8_t>> EngineBinding::Icon(const std::string& key) {
  EnginePage* page = Find(key);
  return page ? page->icon() : std::nullopt;
}
...
void EngineBinding::Flush() {
  ...
  while (!disposing_ && (!queue_.empty() || !due_.empty())) {
    ...
    engine::EngineEvent event = std::move(queue_.front());
    queue_.pop_front();
    const std::vector<uint8_t> bytes = engine::Encode(event);
    const crest_status_t status = report_(app_, engine_, bytes.data(), bytes.size());
