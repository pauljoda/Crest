...
final class ChromiumDownloadAdapter: BrowserEngineDownloadControlling {
    ...
    init(host: any CrestChromiumEngineHost,
        ...
        host.setDownloadDestinationResolver { [weak self] values, reply in
            MainActor.assumeIsolated {
                guard let self, let (id, destination) = self.receive(values) else { reply(nil); return }
                Task { @MainActor in
                    let url = await destination.center.resolveEngineDownloadDestination(id,
    ...
    private func receive(_ values: [String: Any]) -> (BrowserEngineDownloadID, Destination)? {
        guard let profileID = (values["profileId"] as? String).flatMap(UUID.init(uuidString:)),
            let value = values["downloadId"] as? String else { return nil }
        ...
        case "warning":
            guard let token = values["warningToken"] as? String,
                let message = values["message"] as? String else { cancelDownload(id); return nil }
        ...
        destination.center.receiveEngineDownload(BrowserEngineDownloadUpdate(id: id,
            ...
            createdAt: (values["startedAt"] as? NSNumber).map { Date(timeIntervalSince1970: $0.doubleValue) } ?? .now,
