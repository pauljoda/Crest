...
final class BrowserExtensionControllerPoolTests: XCTestCase {
    func testSidebarHostRestoresEachTabsOwnDocumentAndReleasesClosedTabs() async throws {
        ...
        XCTAssertNotNil(host.sidePanelPresentation(forTab: first.id, in: space.id))
        XCTAssertNil(host.sidePanelPresentation(forTab: second.id, in: space.id))
        XCTAssertNil(host.sidePanelPresentation(forTab: first.id, in: SpaceID()))
        ...
        XCTAssertNotNil(host.sidePanelPresentation(forTab: first.id, in: space.id), "The hidden owner's badge remains")
        ...
        XCTAssertNotNil(host.sidePanelPresentation(forTab: second.id, in: space.id))
        ...
        XCTAssertNotNil(other.webView, "Closing one panel preserves the other tab's conversation")
        XCTAssertNil(host.sidePanelPresentation(forTab: first.id, in: space.id))
        XCTAssertNotNil(host.sidePanelPresentation(forTab: second.id, in: space.id))
        browser.selectTab(second.id)
        host.reconcile()
        XCTAssertTrue(host.document === other)
    ...
    private func makePopupReloadProbeHarness() async throws -> PopupReloadProbeHarness {
        let fileManager = FileManager.default
        let rootURL = fileManager.temporaryDirectory.appending(
        ...
        // defaults to.
        let storedResourcePreparer = BrowserStoreWebExtensionStoredResourcePreparer(
            fileManager: fileManager
        )
        let pool = BrowserExtensionControllerPool(
            packageStore: BrowserExtensionPackageStore(
        ...
        let created = try XCTUnwrap(
            pageProvider.createdOffscreenDocuments.first,
            "The fixture worker did not create its offscreen document: \(context.errors.map(\.localizedDescription))"
        )
        XCTAssertTrue(
            offscreenPages.hasExtensionOffscreenDocument(
                extensionBaseURL: context.baseURL,
        ...
        let offscreenDocumentURL = created.url
