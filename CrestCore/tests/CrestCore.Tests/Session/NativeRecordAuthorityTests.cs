...
public sealed partial class BrowserContractsTests {
    ...
    public void OwnedHistoryDeletionUsesLastVisitHalfOpenRangeAndCancelledStoragePreservesHistory() {
        ...
        var core = new NativeSessionAuthority(Bytes(session));
        var before = core.Checkpoint().Read(f.Space.ToString());
        var clear = core.PrepareCommand(SpaceCommand(session, "history.clear", new()));
        using (clear.Reserve()) { }
        Assert.Equal(before, core.Checkpoint().Read(f.Space.ToString()));
        core.PrepareCommand(SpaceCommand(session, "history.remove_range", new() { ["start"] = 10.0, ["end"] = 30.0 })).Commit();
        var retained = JsonNode.Parse(core.Checkpoint().Read(f.Space.ToString()))!.AsArray();
        Assert.Single(retained); Assert.Equal(30.0, retained[0]!["lastVisitedAt"]!.GetValue<double>());
        core.PrepareCommand(SpaceCommand(session, "history.remove_url", new() { ["url"] = "https://example.org/30#ignored" })).Commit();
        Assert.Empty(JsonNode.Parse(core.Checkpoint().Read(f.Space.ToString()))!.AsArray());
    }
    ...
    public void ArchiveRestoreReadsTheOwnedRecordAndCannotRestoreItTwice() {
        ...
        var command = core.PrepareCommand(request); command.Commit();
        ...
        Assert.Empty(JsonNode.Parse(core.Checkpoint().Read("core"))!["spaces"]![0]!["archivedTabs"]!.AsArray());
        Assert.Throws<BrowserRuleException>(() => core.PrepareCommand(request));
    }
    ...
    public void ArchiveRetentionRemovesOnlyTheExpiredOccurrenceOfALegacyRepeatedIdentity() {
        ...
        var command = core.PrepareCommand(Bytes(request)); command.Commit();
    ...
    public void OwnedCleanupPreservesSelectionAndUsesCoreRetentionPreferences() {
        ...
        var command = core.PrepareCommand(SpaceCommand(session, "records.sweep", new(), window: window));
        command.Commit();
        ...
        Assert.Empty(JsonNode.Parse(core.Checkpoint().Read(f.Space.ToString()))!.AsArray());
        Assert.Empty(JsonNode.Parse(core.PrepareCommand(SpaceCommand(session, "records.sweep", new())).Output)!["changes"]!.AsArray());
    }
