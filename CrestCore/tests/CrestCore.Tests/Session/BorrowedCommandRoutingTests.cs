...
public sealed partial class BrowserContractsTests {
    ...
    public void ABorrowedWorkspaceSendsProfileSettingsToItsSourceWhichNormalizesBranding() {
        ...
        var borrowed = JsonNode.Parse(child.Checkpoint().Read("core"))!;
        var branding = new JsonObject { ["colors"] = new JsonArray(), ["bannerStrength"] = 2.0 };

        Assert.Equal(BrowserRuleCodes.BorrowedProfileRequiresOwner, Assert.Throws<BrowserRuleException>(() =>
            child.PrepareCommand(SpaceCommand(borrowed, "space.branding", new() { ["value"] = branding.DeepClone() }))).Code);
        Assert.Equal(BrowserRuleCodes.BorrowedProfile, Assert.Throws<BrowserRuleException>(() =>
            child.PrepareCommand(SpaceCommand(borrowed, "space.reorder", new() {
                ["offsets"] = new JsonArray(0),
                ["destination"] = 1
            }))).Code);

        var applied = owner.PrepareCommand(SpaceCommand(session, "space.branding", new() { ["value"] = branding }));
        applied.Commit();
        ...
        var refreshed = JsonNode.Parse(child.Checkpoint().Read("core"))!;
