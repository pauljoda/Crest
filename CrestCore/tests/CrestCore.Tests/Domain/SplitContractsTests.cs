...
public sealed class SplitContractsTests {
    ...
    private static BrowserSpace Space() => new(Guid.NewGuid(), Guid.NewGuid(), "Splits");
    private static BrowserTab Tab(BrowserSpace space, string title, TabPlacement placement = TabPlacement.Current, Guid? split = null) {
        var tab = BrowserTab.Restore(new(Guid.NewGuid(), TabContent.Web, "https://example.com/" + title, title,
    ...
    public void OpeningFromSavedTabsOrSplitMembersHonorsTheCurrentSectionAndWholeRun() {
        ...
        var fromSaved = new BrowserTab(Guid.NewGuid(), TabContent.Web, "https://example.org", null);
        ...
        var fromSplit = new BrowserTab(Guid.NewGuid(), TabContent.Web, "https://example.org/next", null);
