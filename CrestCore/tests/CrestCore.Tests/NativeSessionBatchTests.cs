...
public sealed partial class BrowserContractsTests
{
    private static JsonObject BatchArguments(JsonNode space, string kind) => new()
    {
        ...
        {
            ["roots"] = new JsonArray(space["tabs"]!.AsArray().Select(t => (JsonNode)new JsonObject
                { ["id"] = t!["id"]!.DeepClone(), ["folder"] = false }).ToArray()),
            ["tabs"] = new JsonArray(space["tabs"]!.AsArray().Select(t => (JsonNode)new JsonObject
            {
                ["id"] = t!["id"]!.DeepClone(), ["placement"] = t["placement"]!.DeepClone(),
                ["folderId"] = t["folderID"]?.DeepClone(), ["splitGroupId"] = t["splitGroupID"]?.DeepClone()
            }).ToArray()),
    ...
    public void OwnedBatchCopiesCurrentMetadataAndRejectsStaleOrCancelledPublication()
    {
        ...
        core.PrepareCommand(1, SpaceCommand(session, "tab.rename", new()
            { ["tabId"] = fixture.Tab.Value.ToString(), ["title"] = "Latest shared name" })).Commit();
    ...
    public void BatchChecksCapturedPlacementAndRefusesTheEntireCrossSpaceMoveAtCapacity()
    {
        ...
        destination["tabs"] = new JsonArray(Enumerable.Range(0, 11).Select(_ =>
        { var tab = second.DeepClone(); tab["id"] = SwiftId(Guid.NewGuid()); return tab; }).ToArray());
        ...
        core.PrepareCommand(1, SpaceCommand(session, "tab.move", new()
            { ["tabId"] = fixture.Tab.Value.ToString(), ["placement"] = "current", ["detach"] = false })).Commit();
