...
public sealed partial class BrowserContractsTests
{
    ...
    public void RestagingRetainsAdditiveFieldsWithoutRevivingClearedFieldsOrRemovedMembers()
    {
        ...
        payload["value"]!["branding"] = new JsonObject {
            ["futureBranding"] = "kept", ["symbolColor"] = new JsonObject { ["red"] = 1.0 },
        ...
        var record = new JsonObject { ["id"] = new JsonObject { ["kind"] = "space", ["value"] = IdForTest(payload["value"]!["id"]!) },
            ["spaceID"] = payload["value"]!["id"]!.DeepClone(), ["payload"] = payload.DeepClone(),
            ["version"] = new JsonObject { ["logicalClock"] = 1UL, ["deviceID"] = Guid.NewGuid().ToString() }, ["futureRecord"] = "kept" };
        var document = JournalDocument(record);
        var journal = new NativeSyncJournal(Bytes(document));
        ...
        var command = JournalCommand(document, "stage", new() { ["payloads"] = new JsonArray(desired),
            ["archiveReasons"] = new JsonArray(), ["deletionReason"] = "explicitDelete", ["now"] = 100.0 });
        ...
        var deleted = updated.Apply(JournalCommand(document, "stage", new() { ["payloads"] = new JsonArray(),
            ["archiveReasons"] = new JsonArray(), ["deletionReason"] = "explicitDelete", ["now"] = 101.0 }));
    ...
    public void AdditiveTabFieldsFollowArchiveAndRestore()
    {
        ...
        var archive = new JsonObject { ["type"] = "archive", ["value"] = new JsonObject {
            ["tab"] = knownTab.DeepClone(), ["archivedAt"] = 200.0, ["reason"] = "closed" } };
        byte[] Stage(JsonObject payload) => JournalCommand(document, "stage", new() {
            ["payloads"] = new JsonArray(payload.DeepClone()), ["archiveReasons"] = new JsonArray(),
            ["deletionReason"] = "superseded", ["now"] = 200.0 });
