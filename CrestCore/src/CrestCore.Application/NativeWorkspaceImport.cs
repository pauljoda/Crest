...
public sealed class NativeWorkspaceImport
{
    ...
    {
        try { return new NativeWorkspaceImport().Apply(session, arguments, mode, now); }
        catch (BrowserRuleException error) { return new() { ["error"] = error.Code }; }
    }
    ...
    public JsonObject Apply(JsonObject source, JsonObject arguments, string mode, double now)
    {
        ...
        { var value = n!.DeepClone().AsObject(); Track(value, i + 1, 0); return value; }).ToArray();
            ...
            foreach (var review in reviews)
            {
                ...
                var edited = additions.Select(tab =>
                {
                    ...
                    if (placement == "pinned" && ++pinned > WorkspaceImportPolicy.MaximumPinnedTabs)
                    {
                        ...
                        {
                            overflowFolder = new JsonObject { ["id"] = SwiftId(Guid.NewGuid()), ["title"] = "Imported Pinned Tabs",
                                ["location"] = "saved", ["symbol"] = "pin.slash", ["isCollapsed"] = false };
        ...
        for (int si = 0; si < spaces.Count; si++)
            foreach (var section in new[] { "tabs", "archivedTabs" })
            {
                ...
                foreach (var node in Items(spaces[si]!, section))
                {
                    ...
                    { ["spaceIndex"] = si, ["tabIndex"] = ti, ["section"] = section,
                        ["sourceIndex"] = origin.Source, ["sourceSpaceIndex"] = origin.Space, ["sourceTabIndex"] = origin.Tab });
                    ti++;
                }
            }
