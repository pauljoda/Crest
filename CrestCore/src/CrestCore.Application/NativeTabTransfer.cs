...
public static class NativeTabTransfer
{
    ...
    {
        LegacySessionDocument Document(JsonObject space) => new(new() { ["session"] = new JsonObject
        { ["spaces"] = new JsonArray(space.DeepClone()), ["selectedSpaceID"] = space["id"]!.DeepClone() } });
        ...
        return new() { ["source"] = Write(sourceDocument, sourceState, a, selected),
