...
public static class NativeSessionMaintenance
{
    ...
    private static TabPlacement StoredPlacement(JsonNode tab) => Text(tab["placement"]) switch
    ...
    {
        ["id"] = SwiftId(ids.Next()), ["title"] = "Start Page", ["symbol"] = "flag.fill",
        ["placement"] = "current", ["lastActivatedAt"] = now, ["keepsPageLoaded"] = false
    };
    ...
    public static JsonObject Repair(JsonObject source, double now, JsonObject? emptySpace = null, IIdSource? ids = null)
    {
        ...
        for (int si = 0; si < spaces.Count; si++)
        {
            ...
            for (int ti = 0; ti < tabs.Count; ti++)
            {
                ...
                assets.Add((JsonNode)new JsonObject { ["spaceIndex"] = si, ["tabIndex"] = ti,
                    ["sourceSpaceID"] = SwiftId(oldSpaceId), ["sourceTabID"] = SwiftId(oldId) });
        ...
        foreach (var node in source)
        {
            ...
            if (groups.TryGetValue(id, out var previous))
                foreach (var (clock, value) in new[] { ("titleModifiedAt", "customTitle"), ("iconModifiedAt", "customIconSymbol"), ("tintModifiedAt", "tint") })
                    if (SyncConflictPolicy.Latest(group[clock]?.GetValue<double>(), previous[clock]?.GetValue<double>()) == 1)
                    { group[clock] = previous[clock]?.DeepClone(); group[value] = previous[value]?.DeepClone(); }
            groups[id] = group;
        }
