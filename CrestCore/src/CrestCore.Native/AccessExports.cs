...
public static unsafe partial class Exports
{
    ...
    public static int AccessBegin(ulong handle, byte* space, byte* profile, int required, ulong* request)
    {
        ...
        *request = 0;
        ...
        catch (BrowserRuleException error) when (error.Code == "authentication_busy") { return CoreStatus.Busy; }
        catch { return CoreStatus.InternalError; }
    }
    ...
    {
        try { return AccessAuthorities.TryRemove(handle, out _) ? CoreStatus.Ok : CoreStatus.InvalidHandle; }
        catch { return CoreStatus.InternalError; }
    }
}
