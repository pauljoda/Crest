...
| Shortcuts, launch and media | The `AdoptShortcuts`, `AssignShortcut`, `ReassignShortcut`, `UnassignShortcut`, `ResetShortcut` and `ResetShortcuts` intents and the `NumberedSelections` query on `crest_app_*`; `launch.plan`; `media.session_event` and `media.arbitrate`; the `OpenTab` intent's `AfterTabId` |
...
| Links, Quick Window, presentation | The `ExternalLinkRoute`, `QuickWindowSite` and `BalancedProtectionRules` queries; `links.route_*`, `links.space_removed`, `quick_window.*`, `page.presentation`, `branding.normalize` and the `SetSpaceBranding` intent |
