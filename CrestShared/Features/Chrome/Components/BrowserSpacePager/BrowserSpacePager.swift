...
#Preview("Space Pager", traits: .fixedLayout(width: 360, height: 240)) {
    @Previewable @State var selectedSpaceID =
        BrowserSpaceBrandingPreviewFixture.simpleSpace.id

    BrowserSpacePager(
        spaces: [
            BrowserSpaceBrandingPreviewFixture.simpleSpace,
            BrowserSpaceBrandingPreviewFixture.crestSpace,
        ],
        selectedSpaceID: selectedSpaceID,
        selectSpace: { selectedSpaceID = $0 },
        content: { space, isSelected in
            ZStack {
                BrowserSpaceBannerBackground(branding: space.branding)
                VStack(spacing: CrestSpacing.medium) {
                    BrowserSpaceIdentityLabel(space: space, iconSize: 28)
                        .font(.headline)
                    Text(isSelected ? "Selected" : "Swipe to select")
                        .font(.caption)
                }
...
#Preview("Space Pager — Locked", traits: .fixedLayout(width: 360, height: 240)) {
    BrowserSpacePager(
        spaces: [
            BrowserSpaceBrandingPreviewFixture.simpleSpace,
            BrowserSpaceBrandingPreviewFixture.crestSpace,
        ],
        selectedSpaceID: BrowserSpaceBrandingPreviewFixture.crestSpace.id,
        ...
        content: { space, isSelected in
            ZStack {
                ...
                VStack(spacing: CrestSpacing.medium) {
                    BrowserSpaceIdentityLabel(space: space, iconSize: 28)
                        .font(.headline)
                    Text(isSelected ? "Drag locked" : "Neighboring Space")
                        .font(.caption)
                }
                .frame(maxWidth: .infinity, maxHeight: .infinity)
            }
