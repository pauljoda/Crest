...
/// It comes from `BrowserSavedFolderLayout`, which reads the tab row profile
/// the same capabilities resolve, so the folder symbol and the favicons beneath
/// it share one column.
struct BrowserSavedFolderHeaderMetrics: Equatable, Sendable {
    ...
    static let pointer = BrowserSavedFolderHeaderMetrics(
        iconWidth: 18,
        iconGlyphSize: nil,
    ...
    static let touch = BrowserSavedFolderHeaderMetrics(
        iconWidth: 20,
        iconGlyphSize: 17,
