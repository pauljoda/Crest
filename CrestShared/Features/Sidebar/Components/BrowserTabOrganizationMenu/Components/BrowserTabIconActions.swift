...
enum BrowserNativeEmojiPickerPresentation: Equatable, Sendable {
...
#if os(macOS)
    /// Keeps AppKit's real text input client alive while the Character Viewer
    /// is frontmost. A SwiftUI `TextField` inside a popover can retain its
    /// `FocusState` while AppKit has already replaced the actual first
    /// responder with the picker button; the system viewer then has nowhere to
    /// insert. Holding the `NSTextField` lets the button restore the native
    /// responder immediately before opening the palette.
    @MainActor
    private final class BrowserNativeEmojiTextInputController {
    ...
    private struct BrowserNativeEmojiTextField: NSViewRepresentable {
