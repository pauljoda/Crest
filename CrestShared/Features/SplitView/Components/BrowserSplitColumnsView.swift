...
#Preview("Split Columns — Three Cards", traits: .fixedLayout(width: 980, height: 560)) {
    @Previewable @State var widthTransaction = BrowserSplitWidthTransaction(
        persistedFractions: [0.4, 0.35, 0.25]
    )

    BrowserSplitColumnsView(
        members: BrowserSplitViewPreviewFixture.members,
        focusedTabID: BrowserSplitViewPreviewFixture.middleTabID,
        frameInsets: BrowserChromeLayout.pageFrameInsets(adjoinsLeadingSidebar: false),
        accent: .indigo,
        ...
        content: { member, isFocused in
            VStack(spacing: CrestSpacing.small) {
                Text(member.title)
                    .font(.headline)
                Text(isFocused ? "Focused card" : "Background card")
                    .font(.caption)
...
#Preview("Split Columns — Carried Card", traits: .fixedLayout(width: 980, height: 560)) {
    @Previewable @State var widthTransaction = BrowserSplitWidthTransaction(
        persistedFractions: [0.34, 0.33, 0.33]
    )

    // The middle card carried to the leading slot: it is still a member, still
    // at its own identity, and drawn as the gap it will drop back into.
    BrowserSplitColumnsView(
        members: BrowserSplitViewPreviewFixture.carriedMiddleToLeadingSlot,
        focusedTabID: BrowserSplitViewPreviewFixture.middleTabID,
        frameInsets: BrowserChromeLayout.pageFrameInsets(adjoinsLeadingSidebar: false),
        accent: .indigo,
        placeholderIndex: nil,
        liftedTabID: BrowserSplitViewPreviewFixture.middleTabID,
        widthTransaction: $widthTransaction,
        onResizeCommit: { _ in },
        onFocus: { _ in },
        usesTransparentInnerSurface: { _ in false },
...
#Preview("Split Columns — Drop Placeholder", traits: .fixedLayout(width: 980, height: 560)) {
    @Previewable @State var widthTransaction = BrowserSplitWidthTransaction(
        persistedFractions: [0.5, 0.5]
    )

    BrowserSplitColumnsView(
        members: BrowserSplitViewPreviewFixture.pair,
        focusedTabID: BrowserSplitViewPreviewFixture.leadingTabID,
        frameInsets: BrowserChromeLayout.pageFrameInsets(adjoinsLeadingSidebar: true),
        accent: .teal,
        placeholderIndex: 1,
        liftedTabID: nil,
        widthTransaction: $widthTransaction,
